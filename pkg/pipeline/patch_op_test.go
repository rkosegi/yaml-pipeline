...
func TestExecutePatchOp(t *testing.T) {
	...
	gd.Set(pp.MustParse("root.sub1.leaf2"), dom.LeafNode("abcd"))
	ps = PatchOpSpec{
		Op:   patch.OpReplace,
		Path: "/root/sub1",
	...
	assert.Equal(t, "xyz", gd.Get(pp.MustParse("root.sub1.leaf2")).AsLeaf().Value())
	...
	assert.NoError(t, New(WithData(gd)).Execute(&ps))
	assert.Equal(t, "abcd", gd.Get(pp.MustParse("root.sub2.leaf3")).AsLeaf().Value())
...
func TestPatchOpAddValue(t *testing.T) {
	...
	gd = dom.ContainerNode()
	gd.Set(pp.MustParse("root.sub.leaf1"), dom.LeafNode("123"))
	ps = PatchOpSpec{
		Op:    patch.OpAdd,
		Path:  "/root/sub/leaf2",
	...
	gd = dom.ContainerNode()
	gd.Set(pp.MustParse("root.sub.leaf3"), dom.LeafNode("aaaa"))
